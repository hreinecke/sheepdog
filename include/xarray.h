 ...
 */
#define XA_FLAGS_LOCK_IRQ	((unsigned)XA_LOCK_IRQ)
#define XA_FLAGS_LOCK_BH	((unsigned)XA_LOCK_BH)
#define XA_FLAGS_TRACK_FREE	((unsigned)4U)
#define XA_FLAGS_ZERO_BUSY	((unsigned)8U)
#define XA_FLAGS_ALLOC_WRAPPED	((unsigned)16U)
#define XA_FLAGS_ACCOUNT	((unsigned)32U)
#define XA_FLAGS_MARK(mark)	((unsigned)(1U << (unsigned)(mark)))
...
void *xa_store(struct xarray *, unsigned long index, void *entry);
 ...
 * Context: Any context.
 */
static inline void xa_init_flags(struct xarray *xa)
{
	sd_init_mutex(&xa->xa_lock);
	xa->xa_head = NULL;
}
...
#define xa_lock(xa)		sd_mutex_lock(&(xa)->xa_lock)
#define xa_unlock(xa)		sd_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_nested(xa, subclass) \
				spin_lock_nested(&(xa)->xa_lock, subclass)

/*
 * Versions of the normal API which require the caller to hold the
 * xa_lock.  They will drop the lock to allocate memory, then reacquire
 * it afterwards.  These functions may also re-enable interrupts if the
 * XArray flags indicate the locking should be interrupt safe.
 */
void *__xa_erase(struct xarray *, unsigned long index);
void *__xa_store(struct xarray *, unsigned long index, void *entry);
void *__xa_cmpxchg(struct xarray *, unsigned long index, void *old,
		void *entry);
int __must_check __xa_insert(struct xarray *, unsigned long index,
		void *entry);
int __must_check __xa_alloc(struct xarray *, uint32_t *id, void *entry,
		struct xa_limit);
int __must_check __xa_alloc_cyclic(struct xarray *, uint32_t *id, void *entry,
		struct xa_limit, uint32_t *next);
			...
			void *old, void *entry)
{
	...
	xa_lock(xa);
	curr = __xa_cmpxchg(xa, index, old, entry);
	xa_unlock(xa);
 ...
 */
static inline __must_check int xa_alloc(struct xarray *xa, uint32_t *id,
		void *entry, struct xa_limit limit)
{
	int err;

	xa_lock(xa);
	err = __xa_alloc(xa, id, entry, limit);
	xa_unlock(xa);
...
static inline int xa_alloc_cyclic(struct xarray *xa, uint32_t *id, void *entry,
		struct xa_limit limit, uint32_t *next)
{
	int err;

	xa_lock(xa);
	err = __xa_alloc_cyclic(xa, id, entry, limit, next);
	xa_unlock(xa);
...
int xa_reserve(struct xarray *xa, unsigned long index)
{
	return xa_err(xa_cmpxchg(xa, index, NULL, XA_ZERO_ENTRY));
}
...
{
	xa_cmpxchg(xa, index, XA_ZERO_ENTRY, NULL);
}
...
bool xas_nomem(struct xa_state *);
