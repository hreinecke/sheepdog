 ...
 * xarray.c
 *
 * copied from linux sources, which have:
 ...
 * Return: true if memory was needed, and was successfully allocated.
 */
static bool __xas_nomem(struct xa_state *xas)
{
	if (xas->xa_node != XA_ERROR(-ENOMEM)) {
		xas_destroy(xas);
		return false;
...
void *__xa_cmpxchg(struct xarray *xa, unsigned long index,
			void *old, void *entry)
{
	return xa_zero_to_null(__xa_cmpxchg_raw(xa, index, old, entry));
}
...
int __xa_insert(struct xarray *xa, unsigned long index, void *entry)
{
	...
	if (!entry)
		entry = XA_ZERO_ENTRY;
	curr = __xa_cmpxchg_raw(xa, index, NULL, entry);
	errno = xa_err(curr);
	if (errno)
		return errno;
		...
		struct xa_limit limit, uint32_t *next)
{
	...
	ret = __xa_alloc(xa, id, entry, limit);
