...
void node_to_json(struct sd_node *node, struct json_object *obj)
{
	...
	json_object_object_add(obj, "zone",
			       json_object_new_uint64(node->zone));
	json_object_object_add(obj, "space",
			       json_object_new_uint64(node->space));
	...
	for (i = 0; i < DISK_MAX; i++) {
		...
		json_object_object_add(disk_obj, "id",
				       json_object_new_uint64(d->disk_id));
		json_object_object_add(disk_obj, "space",
				       json_object_new_uint64(d->disk_space));
...
{
	int i;
	int nr_nodes = 0, nr_gateways = 0, nr_disks = 0;
	...
	for (i = 0; i < logs->nr_nodes; i++) {
		...
		if (flags & SD_CLUSTER_FLAG_DISKMODE) {
			...
			for (disk = 0; disk < DISK_MAX; disk++) {
				if (entry->disks[disk].disk_id == 0)
					break;
			}
			nr_disks++;
		}
	}
	json_object_object_add(obj, "nr_nodes",
			       json_object_new_int(nr_nodes));
	json_object_object_add(obj, "nr_gateways",
			       json_object_new_int(nr_gateways));
	if (flags & SD_CLUSTER_FLAG_DISKMODE)
		json_object_object_add(obj, "nr_disks",
				       json_object_new_int(nr_disks));
}
...
static void json_to_disks(struct json_object *obj, struct sd_node *node)
{
	...
	for (j = 0; j < nr_disks; j++) {
		...
		if (disk_num >= DISK_MAX) {
			sd_warn("disk number %u too large", disk_num);
			continue;
		}
		disk = &node->disks[disk_num];
		id_obj = json_object_object_get(disk_obj, "id");
		if (id_obj)
			disk->disk_id = json_object_get_uint64(id_obj);
		space_obj = json_object_object_get(disk_obj, "space");
		if (space_obj)
			disk->disk_space = json_object_get_uint64(space_obj);
	}
}
#endif
...
void json_to_node(struct json_object *obj, struct sd_node *node)
{
	...
	while (!json_object_iter_equal(&itb, &ite)) {
		...
		if (!strcmp(key, "nid")) {
			...
			node->zone = json_object_get_uint64(val_obj);
		else if (!strcmp(key, "space"))
			node->space = json_object_get_uint64(val_obj);
#ifdef HAVE_DISKVNODES
