...
static int cluster_shutdown(const struct sd_req *req, struct sd_rsp *rsp,
			    void *data, const struct sd_node *sender)
{
	sys_set_status(SD_STATUS_SHUTDOWN);
	if (!node_in_recovery()) {
...
static int local_get_vdi_lock_state(const struct sd_req *req,
				    struct sd_rsp *rsp,
				    void *data, const struct sd_node *sender)
{
	return fill_vdi_lock_state(req, rsp, data);
}
				  ...
				  void *data, const struct sd_node *sender)
{
	...
	add_vdi_state(req->vdi_state.new_vid, req->vdi_state.acl,
		      req->vdi_state.copies, false,
...
#ifdef HAVE_NVMET
	if (ret == SD_RES_SUCCESS &&
	    is_metadata_vid_update(hdr))
		nvmet_notify_member_change(oid_to_vid(oid));
#endif
	return ret;
}
...
static const char *acl_lock_type(uint32_t acl_id)
{
...
{
	struct vdi_lock_state ls;
	const char *lock_type = acl_lock_type(req->vdi_lock.acl);
	int ret = SD_RES_SUCCESS;
...
static struct sd_op_template sd_ops[] = {
	...
	[SD_OP_UNREGISTER_VDI] = {
		...
		.type = SD_OP_TYPE_CLUSTER,
		.process_work = local_release_vdi,
		.process_main = cluster_unregister_vdi_main,
	},
	...
	[SD_OP_GET_VDI_LOCK_STATE] = {
		.name = "GET_VDI_LOCK_STATE",
		.type = SD_OP_TYPE_LOCAL,
		.force = true,
		.process_main = local_get_vdi_lock_state,
	},
